...
//                return Pet.query(on: req)
//                    .join(\Segment.cardID, to: \Pet.id)
//                    //.filter(\Pet.name == "Chiri")
//                    .alsoDecode(Segment.self).all()
//                    .map { results in
//                        results.map { PetAndSegment(pet: $0.0, segments: [$0.1])
//                        }
//                }
//
        
//        return Pet.query(on: req).all().flatMap { pets in
//            let petsResponseFutures = try pets.map { pet in
//                try pet.segments.query(on: req).all().flatMap { segments in
//                    try segments.map { segment in
//                        try segment.attributes.query(on: req).all().map { attributes in
//                            return PetAndSegment(pet: pet, segments: [SegmentAndAttributes(segment: segment, attributes: attributes)])
//                        }
//                    }.flatten(on: req)
//                }
//            }
//            return petsResponseFutures.flatten(on: req)
//        }
//    }
