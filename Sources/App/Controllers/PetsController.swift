...
struct PetsController: RouteCollection {
    func boot(router: Router) throws {
        ...
        petsRoutes.get(Pet.parameter, "segments", use: getSegmentsHandler)
    }
    ...
    func createHandler(_ req: Request, acronym: Pet) throws -> Future<Pet> {
    ...
    // 1
    func getSegmentsHandler(_ req: Request) throws -> Future<[Segment]> { // 2
        return try req.parameters.next(Pet.self) .flatMap(to: [Segment].self) { pet in
            // 3
            try pet.segments.query(on: req).all() }
    }
